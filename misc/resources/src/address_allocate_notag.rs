...
/// Use `RawAddressAllocator` whenever an address range needs to be allocated to different users.
/// An human-readable tag String must also be provided for debugging / reference.
#[derive(Debug, Eq, PartialEq)]
pub struct RawAddressAllocator {
...
impl RawAddressAllocator {
    ...
    fn internal_allocate_with_align(
        ...
        match region {
            Some(slot) => {
                ...
                self.internal_allocate_from_slot(slot, range, tag)
            }
    ...
    pub fn reverse_allocate_with_align(
        ...
        self.internal_allocate_with_align(size, tag, alignment, true)
    }
    ...
    pub fn allocate_with_align(
        ...
        self.internal_allocate_with_align(size, tag, alignment, false)
    }

    pub fn allocate(&mut self, size: u64, tag: String) -> Result<u64> {
        if let Ok(pref_alloc) =
        self.allocate_with_align(size, tag.clone(), self.preferred_align)
        {
            return Ok(pref_alloc);
        }
        self.allocate_with_align(size, tag, self.min_align)
    }
    ...
    pub fn allocate_at(&mut self, range: AddressRange, tag: String) -> Result<()> {
        ...
        {
            Some(&slot) => {
                let _address = self.internal_allocate_from_slot(slot, range, tag)?;
                Ok(())
            }
            None => {
                if let Some(existing_alloc) = self.find_overlapping(range) {
                    Err(Error::ExistingAddrRange(existing_alloc))
                } else {
    ...
    fn find_overlapping(&self, range: AddressRange) -> Option<AddressRange> {
        if range.is_empty() {
            return None;
        }
        ...
        self.allocs
            .iter()
            .find(|(alloc_rage, _)| alloc_range.overlaps(range))
            .map(|(alloc, _)| *alloc)
    }
...
pub struct RawAddressAllocatorSet<'a> {
    allocators: &'a mut [RawAddressAllocator],
}

impl<'a> RawAddressAllocatorSet<'a> {
    pub fn new(allocators: &'a mut [RawAddressAllocator]) -> Self {
        RawAddressAllocatorSet { allocators }
    }
    ...
    pub fn allocate_with_align(
        ...
        for allocator in self.allocators.iter_mut() {
            last_res = allocator.allocate_with_align(size, tag.clone(), alignment);
            if last_res.is_ok() {
                return last_res;
            }
        }
    ...
    pub fn allocate(&mut self, size: u64, tag: String) -> Result<u64> {
        let mut last_res = Err(Error::OutOfSpace);
        for allocator in self.allocators.iter_mut() {
            last_res = allocator.allocate(size, tag.clone());
            if last_res.is_ok() {
                return last_res;
            }
        }
    ...
    pub fn allocate_at(&mut self, range: AddressRange, tag: String) -> Result<()> {
        let mut last_res = Err(Error::OutOfSpace);
        for allocator in self.allocators.iter_mut() {
            last_res = allocator.allocate_at(range,  tag.clone());
            if last_res.is_ok() {
                return last_res;
            }
        }
...
mod tests {
    ...
    fn example() {
        ...
        let mut pool = RawAddressAllocator::new(
            AddressRange {
                start: 0x1000,
                end: 0xFFFF,
            },
            ...
            .unwrap();
        assert_eq!(
            pool.allocate(0x110,  "caps".to_string()),
            Ok(0x1000)
        );
        assert_eq!(
            pool.allocate(0x100,  "cache".to_string()),
            Ok(0x1200)
        );
        assert_eq!(
