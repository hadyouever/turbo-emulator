...
use crate::{mmap::ExternalMapping, AsRawDescriptor, Descriptor, MemoryMapping as CrateMemoryMapping, MemoryMappingBuilder, RawDescriptor, SafeDescriptor, Protection};
use libc::{self, c_int, c_void, MAP_PRIVATE, read, write};
use remain::sorted;
...
impl MemoryMapping {
    ...
    /// Creates an anonymous shared mapping of `size` bytes with `prot` protection.
    /// For user mode emulation purposes
    /// # Arguments
    /// * `size` - Size of memory region in bytes.
    /// * `prot` - Protection (e.g. readable/writable) of the memory region.
    pub fn new_protection_usermode(size: usize, prot: Protection) -> Result<MemoryMapping> {
        // This is safe because we are creating an anonymous mapping in a place not already used by
        // any other area in this process.
        unsafe {
            MemoryMapping::try_mmap(
    ...
    pub fn from_fd_offset_flags(
        ...
        unsafe {
            ...
            MemoryMapping::try_mmap(None, size, prot.into(), flags, Some((fd, offset)), system_mode)
        }
    }
    ...
    pub fn from_fd_offset_protection(
        ...
        MemoryMapping::from_fd_offset_flags(fd, size, offset, libc::MAP_SHARED, prot, true)
    }
    ...
    pub fn from_fd_offset_protection_populate(
        ...
        MemoryMapping::from_fd_offset_flags(fd, size, offset, flags, prot, true)
    }
...
impl MemoryMappingArena {
    ...
    pub fn new_protection_usermode(size: usize) -> Result<MemoryMappingArena> {
        // Reserve the arena's memory using an anonymous read-only mmap.
        let mut res: MemoryMappingArena = MemoryMapping::new_protection_usermode(size, Protection::read_write_execute()).map(From::from)?;
        res.is_usermode = true;
        Ok(res)
    }
    pub fn new_protection_fixed_usermode(size: usize, addr: *mut u8) -> Result<MemoryMappingArena> {
        // Reserve the arena's memory using an anonymous read-only mmap.
        let mut res: MemoryMappingArena = MemoryMapping::new_protection_fixed(addr, size, Protection::read_write_execute(), false).map(From::from)?;
        res.is_usermode = true;
        Ok(res)
    }
    ...
    pub fn add_fd_offset(
        ...
        self.add_fd_offset_protection(offset, size, fd, fd_offset, Protection::read_write_execute())
    }
    ...
    fn try_add(
        ...
        let mmap = unsafe {
            match fd {
                ...
                None => MemoryMapping::new_protection_fixed(self.addr.add(offset), size, prot, !self.is_usermode)?,
            }
        };
...
impl From<MemoryMapping> for MemoryMappingArena {
    fn from(mmap: MemoryMapping) -> Self {
        ...
        std::mem::forget(mmap);
        MemoryMappingArena { addr, size, is_usermode: false }
    }
}
